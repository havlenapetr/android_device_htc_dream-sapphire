...
static const struct sensor_t sSensorList[] = {
        { "AK8976A 3-axis Accelerometer",
                ...
                SENSOR_TYPE_ACCELEROMETER, 2.8f*9.81f, 9.81f/45.0f, 3.0f, 0, { } },
        ...
        struct hw_device_t** device)
{
	...
	__android_log_print(ANDROID_LOG_INFO, 
        "SapphireSensors", "(device open) name: %s", name);
	...
	__android_log_print(ANDROID_LOG_INFO, "SapphireSensors", 
        "(device open) return status: %s", strerror(-status));
    return status;
}
